...
query = "SELECT * FROM analysis_db.ublox_sinua order by ts desc limit 100"
...
fig = plt.figure()
fig.suptitle('long & lat', fontweight='bold')
gs = gridspec.GridSpec(2, 2)
# labels = df_new.ts
# labels = ['\n'.join(wrap(l, 11)) for l in labels]

plt.subplot(gs[0,0])
plt.plot(df_new.ts, df_new.lat2, "green", marker='o')
plt.ylabel('latitude, °', fontweight='bold')
plt.grid(color = 'gray', linestyle = '--', linewidth = 0.5)
plt.xticks(color='w')

plt.subplot(gs[0,1])
plt.plot(df_new.ts, df_new.lon2, "blue")
