...
#     dfmean = dff[['lat','lon','msl']].\
#             rolling(min_periods=1,window=6,center=False).mean()
#     dfsd = dff[['lat','lon','msl']].\
#             rolling(min_periods=1,window=6,center=False).std()

#     dfulimits = dfmean + (1.5*dfsd)
#     dfllimits = dfmean - (1.5*dfsd)

#     dff.lat[(dff.lat > dfulimits.lat) | \
#         (dff.lat < dfllimits.lat)] = np.nan
#     dff.lon[(dff.lon > dfulimits.lon) | \
#         (dff.lon < dfllimits.lon)] = np.nan
#     dff.msl[(dff.msl > dfulimits.msl) | \
#         (dff.msl < dfllimits.msl)] = np.nan

#     dflogic = dff.lat * dff.lon * dff.msl
#     dff = dff[dflogic.notnull()]

#     return dff

# df2 = outlier_filter(df2)
# print('df2 outlier:', len(df2))
...
# #df_new_mva.drop(df_new_mva.tail(2).index,inplace=True)
...
# #PLOT
# fig = plt.figure()
# fig.suptitle('rover in stable position', fontweight='bold')
